...
namespace Ex._07___wpf
{
    class loja
    {
        ...
        {
            veiculo[] lf = veiculos.OrderBy(veiculo => veiculo.Fabricante).ThenBy(veiculo => veiculo.Modelo).ToArray();
            return lf;
        }

        public veiculo[] ListarPorModelo()
        {
            veiculo[] lm = veiculos.OrderBy(veiculo => veiculo.Modelo).ThenByDescending(veiculo => veiculo.Preco).ToArray();
            return lm;
        }
